...
utils.defineProperty(Wallet, 'RNCreateRandom', function(options){
  var entropy = utils.randomBytes(16);
  ...
  var mnemonic = HDNode.entropyToMnemonic(entropy);
  return Wallet.RNFromMnemonic(mnemonic, options.path);
});


utils.defineProperty(Wallet, 'RNFromMnemonic', function(mnemonic, path) {
  if (!path) { path = defaultPath; }


  var RNhdnode = HDNode.RNFromMnemonic(mnemonic);
